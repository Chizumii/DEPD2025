...
class NetworkApiServices implements BaseApiServices {
  ...
  Future<dynamic> getApiResponse(String endpoint) async {
    try {
      // Perbaikan URL Parsing sudah diterapkan di sini (Uri.parse)
      ...
      final response = await http.get(
        uri,
        headers: {
          // HAPUS Content-Type dari GET request (hanya perlu API Key)
          'key': Const.apiKey,
        },
      );
  ...
  Future<dynamic> postApiResponse(String endpoint, dynamic data) async {
    try {
      // PERBAIKAN: Gunakan Uri.parse untuk merakit URL penuh (konsistensi POST).
      final uri = Uri.parse('https://${Const.baseUrl}${Const.subUrl}$endpoint');
      ...
      final response = await http.post(
        uri,
        headers: {
          // Header ini penting untuk POST form-urlencoded
